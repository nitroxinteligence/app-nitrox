...
void main() {    
   ...
   vec3 color = hsl2rgb(
       0.35 + n * 0.1, // Shifted hue to green range (0.35 is in the green spectrum)
       0.8,            // Increased saturation for vibrant greens
       0.5            // Adjusted lightness
   );
...
void main() {
   vec3 light = vec3(0.0);
   vec3 skyColor = vec3(0.345, 0.91, 0.467);    // #58E877
   vec3 groundColor = vec3(0.184, 0.478, 0.247); // #2F7A3F
...
void main() {
   ...
   // Modified color calculation to use green tones
   v_color = hsv2rgb(vec3(
       0.35 + noise * 0.1, // Base hue shifted to green (0.35)
       0.8,                // High saturation for vivid colors
       0.7                 // Slightly increased brightness
   ));
