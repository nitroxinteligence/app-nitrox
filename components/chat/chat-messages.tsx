...
function FileAttachment({ file }: FileAttachmentProps) {
  ...
  return (
    <a
      ...
      className={cn(
        "flex items-center gap-2 bg-[#1a1a1c] text-white text-xs px-3 py-2 rounded hover:bg-[#272727] transition-colors",
            ...
            {message.role === "user" ? (
              <div className="relative flex justify-end">
                {message.id && editingMessageId === message.id ? (
                  <div className="w-1/2 bg-[#1E1E1E] rounded-lg p-3">
                    <textarea
                      value={editContent}
                      onChange={(e) => setEditContent(e.target.value)}
                      className="w-full bg-[#1E1E1E] text-white rounded-lg p-2 min-h-[44px] resize-none border border-[#272727] focus:outline-none focus:border-[#58E877]"
                      onKeyDown={(e) => {
                  ...
                  <div className="w-1/2 text-[#E8F3ED] bg-[#1E1E1E] rounded-lg p-3 pb-10">
              ...
              <div className="relative">
                <div className="text-white bg-[#1E1E1E] rounded-lg p-3 pb-10">
                  <p className="whitespace-pre-wrap break-words">{message.content}</p>
                  {message.attachments && message.attachments.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {message.attachments.map((file, index) => (
